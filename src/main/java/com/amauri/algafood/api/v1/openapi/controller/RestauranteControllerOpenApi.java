...
public interface RestauranteControllerOpenApi {
    ...
    @Operation(summary = "Cadastra um restaurante", responses = @ApiResponse(
            responseCode = "201",
            description = "Restaurante cadastrado"
    ))
    RestauranteModel salvar(@RequestBody(description = "Representação de um novo restaurante", required = true)
    ...
    RestauranteModel atualizar(@Parameter(description = "ID de um restaurante", example = "1", required = true)
                               Long restauranteId,
    ...
    ResponseEntity<Void> ativarMultiplos(@RequestBody(description = "IDs de restaurantes", required = true)
                                         List<Long> restauranteIds);

    @Operation(summary = "Inativa múltiplos restaurantes", responses = {
            @ApiResponse(responseCode = "204", description = "Restaurantes ativados com sucesso"),
    })
    ResponseEntity<Void> inativarMultiplos(@RequestBody(description = "IDs de restaurantes", required = true)
                                           List<Long> restauranteIds);
